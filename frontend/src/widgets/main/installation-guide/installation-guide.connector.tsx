...
export const InstallationGuideConnector = (props: IProps) => {
  ...
  const [selectedPlatform, setSelectedPlatform] =
    useState<TSubscriptionPagePlatformKey>(() => {
    ...
    return (
      <Group gap="xs" wrap="wrap">
        {buttons.map((button, index) => {
          // Кнопка, добавляющая подписку в приложение, — главное действие
          // шага. Обводка читается как ссылка, и её не нажимают: делаем
          // заливной и во всю ширину, остальные оставляем как были.
          const isPrimary = button.type === "subscriptionLink";
          ...
          return (
            <Button
              ...
              variant={isPrimary ? "filled" : variant}
            >
              {t(button.text)}
            </Button>
          );
        ...
        {platformApps.length > 0 && (
          <Box>
            <div className={classes.appsGrid}>
              {platformApps.map(
                (app: TSubscriptionPageAppConfig, index: number) => {
